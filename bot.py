...
def get_weather(city):
    ...
    if forecast_data.get("cod") == "200":
        ...
        rain_chance = int(
            forecast_data["list"][0].get("pop", 0) * 100
        )
...
def make_weather_text():
    ...
    for display_name, city in cities:
        ...
        try:
            ...
            text += (
                f"{display_name}\n"
                f"🌡️ {temp}°C ｜ ☔ {rain_chance}%\n"
                f"🔺{temp_max}°C ｜ 🔻{temp_min}°C\n"
                f"☁️ {weather} ｜ AQI {aqi_text(aqi)}\n\n"
            )
